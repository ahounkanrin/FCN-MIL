...
class FCN8VGG:
    ...
    def build(self, rgb, train=False, num_classes=21, random_init_fc8=False,
              debug=False, use_dilated=False, upsample_by_deconv=False):
        """
...
#            assert red.get_shape().as_list()[1:] == [513, 513, 1]
#            assert green.get_shape().as_list()[1:] == [513, 513, 1]
#            assert blue.get_shape().as_list()[1:] == [513, 513, 1]
            bgr = tf.concat([
                blue - VGG_MEAN[0],
                green - VGG_MEAN[1],
                red - VGG_MEAN[2],
...
#        return self.upscore32
    ...
    def _fc_layer(self, bottom, name, num_classes=21,
...
#            stddev = (2 / num_input)**0.5
    ...
    def get_deconv_filter(self, f_shape):
        ...
        init = tf.constant_initializer(value=weights,
                                       dtype=tf.float32)
        with tf.variable_scope("up_filter", reuse=tf.AUTO_REUSE):
...
#                              shape=weights.shape)
    ...
    def _variable_with_weight_decay(self, shape, stddev, wd, decoder=False):
        ...
        with tf.variable_scope("weights", reuse=tf.AUTO_REUSE):
            var = tf.get_variable(name="weights", initializer=initializer, shape=shape)
...
#                              initializer=initializer)
    ...
    def _bias_variable(self, shape, constant=0.0):
        ...
        with tf.variable_scope("biases", reuse=tf.AUTO_REUSE):
            var = tf.get_variable(name="biases", initializer=initializer, shape=shape)
...
#                              initializer=initializer)
    ...
    # -----------------------These methods are adapted from deeplab-lfov-------------------------------------------------------------------
    ...
    def preds(self, input_batch):
        ...
        """
        raw_output = self.build(tf.cast(input_batch, tf.float32), train=False, num_classes=21, random_init_fc8=True, upsample_by_deconv=False)
        raw_output = tf.image.resize_bilinear(raw_output, tf.shape(input_batch)[1:3,]) # Upsample the raw output bilinearly to match the size of the input
    ...
    def loss(self, img_batch, label_batch):
        ...
        """
        raw_output = self.build(tf.cast(img_batch, tf.float32), train=True, num_classes=21, random_init_fc8=True)
        ...
        # Pixel-wise softmax loss.
        loss = tf.nn.softmax_cross_entropy_with_logits_v2(logits=prediction, labels=gt)
    ...
    def mil_loss(self, data_batch, is_training=False):
        
        img_batch, label_batch = data_batch
        batch_size = label_batch[:,0,0,0]
        batch_size = batch_size.get_shape()
        batch_size = batch_size.num_elements()
        print("\n\nBATCH SIZE:", batch_size)
        
        raw_output = self.build(tf.cast(img_batch, tf.float32), train=is_training, num_classes=21, random_init_fc8=True, upsample_by_deconv=False)
#        print("\n\n RAW OUTPUT SHAPE: ", raw_output)
        raw_output = tf.image.resize_bilinear(raw_output, tf.shape(img_batch)[1:3,]) # Upsample the raw output bilinearly to match the size of the input
        probabilities=tf.nn.softmax(raw_output)

        prediction = tf.reshape(probabilities, [batch_size, -1, n_classes])
        
        # Need to resize labels to have the same size as the output image
        #label_batch = self.prepare_weak_label(label_batch, tf.stack(raw_output.get_shape()[1:3]))
        
        label = tf.squeeze(label_batch, axis=[3])
        loss = tf.Variable(0.0, trainable=False)
        
        # Calculate the loss for each batch
        for label_number in range(batch_size):
            ...
            loss_per_image_vector = tf.map_fn(lambda i: tf.log(tf.reduce_max(prediction[label_number,:,i])), weak_label, dtype=tf.float32)
            #print("This is the loss_per_image_vector: ", loss_per_image_vector)
            ...
            loss_per_image = tf.multiply(-1.0, loss_per_image)
            loss = loss + loss_per_image
        loss = tf.divide(loss, batch_size) # Averaged loss for the batch
